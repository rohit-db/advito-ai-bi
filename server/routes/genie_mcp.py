...
# The two managed Genie MCP server shapes the toggle switches between.
...
# Genie's genie_ask markdown wraps embedded result tables in HTML-comment
# markers like `<!-- begin-embedded:query_48e0a3 -->` ... `<!-- end-embedded:... -->`.
# We render the table separately (structured), so strip these blocks from the prose.
    ...
    for i in range(len(lines) - 1):
        ...
        for r in rows:
            ...
            for i, v in enumerate(r):
                if i < len(numeric_flags) and numeric_flags[i] and _is_number(v):
                    cells.append(_format_number(_NUMERIC_STRIP_RE.sub("", v)))
                else:
                    cells.append(v)
...
def _resolve_tool_names(tools: list, space_id: Optional[str] = None) -> tuple[Optional[Any], Optional[Any]]:
    """Map discovered tools to ask/poll roles (canonical names + heuristics).

    Handles both `genie_ask`/`genie_poll_response` (multi-space, generic) and
    `query_space_<id>`/`poll_response_<id>` (per-space). When `space_id` is given
    and the server exposes a tool per space (the workspace-wide server returns
    one `query_space_<id>` per accessible space), prefer the tool for our space.
    """
    by_name = {t.name: t for t in tools}
    names = list(by_name.keys())

    def _is_poll(n: str) -> bool:
        return bool(re.search(r"poll", n, re.I))
...
def _tool_call_result(state: dict) -> dict:
...
async def health(request: Request):
    ...
    """
    mode = _normalize_mode(request.query_params.get("mode"))
    ...
    try:
        ...
        async with streamablehttp_client(server_url, headers=headers) as (read, write, _):
            async with ClientSession(read, write) as session:
                ...
                ask_tool, poll_tool = _resolve_tool_names(tools, space_id)
            ...
            async with streamablehttp_client(server_url, headers=headers) as (read, write, _):
                async with ClientSession(read, write) as session:
                    ...
                    yield _sse({
                        ...
                        "tool": ask_tool.name,
                        "args": ask_args,
                        "result": _tool_call_result(state),
                    })
                    ...
                    if poll_tool and message_id and status not in _TERMINAL_STATUSES:
                        ...
                        while attempt < POLL_MAX_ATTEMPTS:
                            ...
                            yield _sse({
                                ...
                                "attempt": attempt,
                                "args": poll_args,
                                "result": _tool_call_result(pstate),
                            })
