...
**SDK embedding may also work with this path.** The `@databricks/aibi-client` SDK supports `hideDatabricksLogo` and `navigate()` for page switching, but requires a properly scoped token (it didn't work in the Databricks App because there's no embedding user scope for OBO tokens). With the external hosting approach and SP-minted scoped tokens (3-step OAuth exchange), the SDK should authenticate correctly — which would give us logo hiding + page navigation natively. Filter passing via the SDK is on the Q4 roadmap per the AI/BI PM, but in the meantime our URL parameter approach can be combined with the SDK (we validated the SDK builds the same `/embed/` iframe URL internally). This needs testing with a proper scoped token but is a viable path.
