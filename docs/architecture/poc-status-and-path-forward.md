...
- **Deployed and running** as a Databricks App: https://advito-ai-bi-1048934788948873.aws.databricksapps.com
...
**Firefly Analytics** (https://www.firefly-analytics.com) is a "Built on Databricks" reference blueprint that solves exactly this.

Architecture docs: https://www.firefly-analytics.com/docs/architecture/overview
...
**Key point: everything we've built is reusable.** Our React UI, FastAPI backend, Genie integration, custom agent, and metric view all work the same — the only addition is the proxy layer (~200 lines of code). The SP management (one per customer) is automatable — Firefly's blueprint includes patterns for credential generation, encryption, and rotation.

## Recommendation

Go with **Path 1 (Databricks Apps) for MVP** with a limited customer set. It's the fastest route, everything works, and we avoid the infrastructure overhead. In parallel, build the proxy layer following the Firefly blueprint so we're ready for **Path 2 when we scale** to the full 600-800 user base where white-labeling and zero Databricks provisioning become hard requirements.

## Reference

- **Live POC:** https://advito-ai-bi-1048934788948873.aws.databricksapps.com
- **Firefly Architecture:** https://www.firefly-analytics.com/docs/architecture/overview
- **Firefly Apps Proxy:** https://www.firefly-analytics.com/docs/solutions/embedding-apps
