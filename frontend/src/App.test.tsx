 ...
 *   - an unlabelled section containing "Back to Prism"
...
describe("App — admin-context sidebar builder", () => {
  ...
  it("renders 'Back to Prism' in the sidebar when operator is at /admin/*", async () => {
    renderAppAtAdmin();
    // useUser is async; wait for the operator user to load and sidebar to rebuild.
    await waitFor(() => {
      expect(screen.getByText(/back to prism/i)).toBeInTheDocument();
    });
  });
