...
export default function GenieSqlBlock({
  ...
  const accent = compact ? "text-indigo-700" : "text-violet-700";
  const label = compact ? "SQL" : "Generated SQL";
  const icon = compact ? "w-3.5 h-3.5" : "w-4 h-4";
  const labelText = compact ? "text-[11px]" : "text-xs";
  const pre = compact
    ? "mt-2 bg-gray-900 text-green-300 p-2.5 rounded-lg text-[10px] font-mono overflow-x-auto"
    : "mt-2 bg-gray-900 text-green-300 p-3 rounded-lg text-[11px] font-mono overflow-x-auto";
