...
import { Send, Loader2, Trash2, Code, Sparkles } from "lucide-react";
...
export default function ChatPanel({
  ...
  const hasFilters = !!filterContext && filterContext !== "No filters applied";
  const suggestions = PAGE_SUGGESTIONS[activePath] ?? DEFAULT_SUGGESTIONS;

  // Build context string for Genie
  const fullContext = [
    `Dashboard: ${activePageLabel}`,
    filterContext,
  ]
    .filter(Boolean)
    .join(". ");
  ...
  return (
    <Sheet open={isOpen} onOpenChange={(open) => !open && onClose()}>
      ...
      >
        {/* Gradient header */}
        <div className="bg-gradient-to-r from-indigo-600 to-purple-600 px-4 pt-5 pb-4 shrink-0">
          ...
          {messages.map((msg, i) => (
            ...
            >
              ...
              >
                ...
                {showSql === i && msg.sql && (
                  <pre className="mt-1.5 p-2 bg-gray-900 text-green-300 rounded text-[11px] overflow-auto max-h-32">
                    {msg.sql}
                  </pre>
                )}
